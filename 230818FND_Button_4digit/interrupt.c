...
{
	if(data>=10000) {	data=9999; }// 10000이 넘어가면 data에 9999까지 나오게 설정 // fndDisplayData=data;
	fndDisplayData=data; 
}

uint16_t FND_getFndDisplayData(){ return fndDisplayData; }//변수에 저장된 데이터 반환 	

void FND_selectDigit(uint8_t digit) //자릿수 선택  // 에러나도 코드 작동 잘되서 4 DIGIT FND와 2buttons에 반영잘됨
{	FND_DIGIT_PORT|=0xf0;
